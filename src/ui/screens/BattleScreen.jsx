...
// /* -----------------------------
//    Forced-switch auto-resolve
// ------------------------------ */

// function aliveIndexes(team) {
//   const res = [];
//   for (let i = 0; i < (team ?? []).length; i++) {
//     if ((team[i]?.hp ?? 0) > 0) res.push(i);
//   }
//   return res;
// }

// function autoResolveForcedSwitch(next) {
//   if (!next.mustSwitchPlayer) return;

//   const alive = aliveIndexes(next.playerTeam);
//   if (alive.length === 1) {
//     const only = alive[0];
//     next.playerActive = only;
//     next.mustSwitchPlayer = false;
//     next.switchMode = null;
//     next.pendingSwitch = null;
//     next.waitingPlayer = true;
//     next.log = [...next.log, `${next.playerTeam[only].name}, à toi !`];
//   }
// }
...
//   // si on est forcé de switch mais qu'il ne reste qu'UN seul Pokémon vivant, on le met actif automatiquement.
//   useEffect(() => {
//     if (animating) return;
//     if (state.phase === "ended") return;
//     if (!state.mustSwitchPlayer) return;

//     const alive = aliveIndexes(state.playerTeam);
//     if (alive.length === 1) {
//       setState((prev) => {
//         const next = structuredClone(prev);
//         autoResolveForcedSwitch(next);
//         return next;
//       });
//     }
//   }, [state.mustSwitchPlayer, animating, state.phase, state.playerTeam]);
...
//   function startAnimatedTransition(prev, final) {
//     // au cas où final tombe sur un "mustSwitch" avec 1 seul vivant, on évite le softlock
//     const fixedFinal = structuredClone(final);
//     autoResolveForcedSwitch(fixedFinal);

//     const steps = buildAnimatedSteps(prev, fixedFinal);
//     if (!steps.length) {
//       setState(fixedFinal);
//       return;
//     }
...
//         autoResolveForcedSwitch(next); // (normalement jamais auto ici car l'engine ne met pendingSwitch que s'il y a du banc)
//         startAnimatedTransition(prev, next);
//         return prev;
//       }

//       // ✅ Si le Pokémon actif avant est KO et que l'engine a auto-switch
//       const wasKO = next.playerTeam[prevActive]?.hp <= 0;
//       const engineAutoSwitched = next.playerActive !== prevActive;
...
//         // s'il reste plusieurs choix -> on force le choix (et on garde le KO affiché)
//         if (alive.length > 1) {
//           next.playerActive = prevActive; // revenir sur le KO pour l'UI
//           next.mustSwitchPlayer = true;
//           next.switchMode = "ko";
...
//         // ✅ s'il ne reste qu'un seul vivant -> on accepte l'auto-switch et on débloque
//         next.mustSwitchPlayer = false;
//         next.switchMode = null;
//         next.pendingSwitch = null;
//         next.waitingPlayer = next.phase === "ended" ? false : true;
//         next.log = [...next.log, `${next.playerTeam[next.playerActive].name}, à toi !`];
...
export default function BattleScreen({ run, dispatch }) {
  ...
  function doSwitch(toIndex) {
    ...
    // ✅ MODE KO : on peut cliquer l'actif pour CONFIRMER (débloque les attaques)
    if (state.mustSwitchPlayer && state.switchMode === "ko") {
      setState((prev) => {
        const next = structuredClone(prev);

        if (toIndex !== prev.playerActive) {
          next.playerActive = toIndex;
          next.log = [...next.log, `${next.playerTeam[toIndex].name}, à toi !`];
        } else {
          next.log = [...next.log, `${next.playerTeam[toIndex].name} reste en place !`];
        }
    ...
    // (après le bloc KO, on remet le guard "actif")
    if (toIndex === state.playerActive) return;
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {state.playerTeam.map((pk, idx) => {
              ...
              // ✅ en mode KO : on autorise le clic sur l'actif (pour "confirmer")
              const disabled =
                animating ||
                pk.hp <= 0 ||
                (isActive && !(state.mustSwitchPlayer && state.switchMode === "ko")) ||
                (!state.waitingPlayer && !state.mustSwitchPlayer);
